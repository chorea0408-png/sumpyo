...
import type { ProjectExpense, ProjectExpenseCategory, ProjectId } from '../types';
import { PROJECT_EXPENSE_CATEGORIES, projectExpenseCategoryLabel } from '../data/projectExpenseCategories';
import { fmtDateShort, toDateInput } from '../lib/date';

interface Props {
  projectId: ProjectId;
  expenses: ProjectExpense[];
  onAdd: (date: string, category: ProjectExpenseCategory, title: string, amount: number) => void;
  onRemove: (id: string) => void;
}

export default function ProjectExpenseTracker({ projectId, expenses, onAdd, onRemove }: Props) {
  const [adding, setAdding] = useState(false);
  const [date, setDate] = useState(() => toDateInput(new Date()));
  const [category, setCategory] = useState<ProjectExpenseCategory>('lodging');
          ...
          <ul className="expense-list">
            {own.map((e) => (
              <li key={e.id} className="expense-row">
                ...
                <span className="expense-category">{projectExpenseCategoryLabel(e.category)}</span>
      ...
      {adding ? (
        <div className="template-add-card">
          ...
          <select
            className="date-input full"
            value={category}
            onChange={(e) => setCategory(e.target.value as ProjectExpenseCategory)}
            aria-label="지출 카테고리"
          >
